...
(function (global, factory) {
  ...
  function compareTouchList(a, b) {
    ...
    for (var i, len = a.length; i < len; i++) {
      ...
      if (
        aItem.pageX !== bItem.pageX ||
        aItem.pageY !== bItem.pageY ||
        aItem.clientX !== bItem.clientX ||
        aItem.clientY !== bItem.clientY
      )
        return false;
    }
  ...
  function compareEventInAlipay(last, now) {
    ...
    return (
      compareEventProperty('detail', last, now) ||
      compareEventProperty('touches', last, now) ||
      compareEventProperty('changedTouches', last, now)
    );
  }
  ...
  var BUILTIN_COMPONENT_LIST = new Set([
    'movable-view',
    'cover-image',
    'cover-view',
    'movable-area',
  ...
  /**
   * A data of supported style properties that by default contain only commonly used style properties
   */
  var styleList = {
    position: 'position',
    top: 'top',
        ...
        internal.$batchedUpdates(function () {
          _this3.renderStacks.forEach(function (task, index) {
            ...
            if (task.type === 'children') {
              ...
              internal.$spliceData(
                ((_internal$$spliceData = {}),
                (_internal$$spliceData[task.path] = task.item ? spliceArgs.concat(task.item) : spliceArgs),
                _internal$$spliceData),
                callback,
              );
            } else {
              ...
              internal.setData(
                ((_internal$setData = {}), (_internal$setData[task.path] = task.value), _internal$setData),
                callback,
              );
            }
          });
    ...
    _proto.getElementsByClassName = function getElementsByClassName(className) {
      ...
      cache.getAllNodes().forEach(function (element) {
        ...
        if (
          element &&
          element._isRendered() &&
          element.__pageId === _this3.__pageId &&
          classNames.every(function (c) {
            return element.classList && element.classList.contains(c);
          })
        ) {
